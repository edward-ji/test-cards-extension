...
const gateways: { id: string; readyText: string; timeout?: number }[] = [
    ...
    { id: 'ebanx',           readyText: '6362 9700 0045 7013', timeout: 10000 },
...
test.describe('gateway screenshots', () => {
    ...
    for (const { id, readyText, timeout } of gateways) {
        test(`${id}`, async ({ page }) => {
            await page.locator('#gatewaySelector').selectOption(id);
            await expect(page.locator('#cards')).toContainText(readyText, { timeout: timeout ?? 5000 });
