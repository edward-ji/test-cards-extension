...
test.describe('favorites', () => {
    ...
    test('add hides empty message', async ({ page }) => {
        await page.locator('tr').filter({ hasText: CARD_1 }).locator('.fav-icon').click();
    ...
    test('card removed from main list', async ({ page }) => {
        await page.locator('tr').filter({ hasText: CARD_1 }).locator('.fav-icon').click();
        // After favoriting, the main section no longer has a row with .fav-icon for this card
        await expect(
            page.locator('tr').filter({ hasText: CARD_1 }).locator('.fav-icon')
        ).toHaveCount(0);
    });
    ...
    test('remove restores empty message', async ({ page }) => {
        ...
        await page.locator('tr').filter({ hasText: CARD_1 }).locator('.unfav-icon').click();
        await expect(page.locator('text=' + ADD_TO_FAVS_TEXT)).toBeVisible();
    });
    ...
    test('multiple favorites', async ({ page }) => {
        await page.locator('tr').filter({ hasText: CARD_1 }).locator('.fav-icon').click();
        await page.locator('tr').filter({ hasText: CARD_2 }).locator('.fav-icon').click();
        await expect(page.locator('#tableFavouritesId')).toContainText(CARD_1);
